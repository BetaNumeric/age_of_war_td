...
function detectMobileDevice() {
  ...
  const hasMobileUa = /android|iphone|ipad|ipod|mobile/i.test(ua);
  const coarsePointer = window.matchMedia && window.matchMedia('(pointer: coarse)').matches;
  const smallViewport = Math.min(window.innerWidth || 0, window.innerHeight || 0) <= 1024;
  return hasMobileUa || (coarsePointer && smallViewport);
}
...
function showA2HSBanner({ showInstallButton, showOpenButton, text: messageText }) {
  ...
  banner.style.paddingTop = 'calc(14px + env(safe-area-inset-top))';
